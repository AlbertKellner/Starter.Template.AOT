...
FASE 3 — ATUALIZAR DURANTE EXECUÇÃO
  ...
  5. Se um comportamento é bloqueado por ferramenta temporariamente indisponível (MCP ainda não conectou, Docker DNS, rede):
     - NÃO remover do TodoWrite — manter como pending com nota de bloqueio
     - NÃO declarar "indisponível" prematuramente — MCP servers têm inicialização assíncrona e podem demorar a conectar
     - Prosseguir com outros passos do pipeline que não dependem da ferramenta
     - Se a ferramenta reconectar durante a sessão (system-reminder com deferred tools), retomar imediatamente
  ...
  2. Para cada comportamento NÃO marcado como completed:
     a. Classificar o motivo:
        ...
        (iii) Bloqueado por erro — registrar em bash-errors-log.md
              Exemplo: passo 10 bloqueado por MCP ainda não conectado → verificar
              se MCP conectou durante a sessão (system-reminder com deferred tools);
              se sim, executar agora; se não, registrar como (iii) com justificativa
...
| 2026-04-02 | Corrigido: Fase 3 item 5 — "MCP indisponível" substituído por "MCP ainda não conectou" (inicialização assíncrona); adicionada instrução de não declarar indisponibilidade prematuramente | Diagnóstico de MCP — testes exaustivos |
