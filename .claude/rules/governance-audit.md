...
| 37 | Tipo de servidor MCP no `.mcp.json` (`http`/`stdio`) corresponde ao documentado em `technical-overview.md` (tabela Recursos Operacionais) | Consistência infra↔governança |
| 38 | Variáveis de ambiente referenciadas no `.mcp.json` (`${VAR}`) estão documentadas em `required-vars.md` | Consistência infra↔governança |
...
| 2026-03-31 | Adicionado: checks #37 e #38 — consistência infra↔governança: tipo de servidor MCP no `.mcp.json` vs. `technical-overview.md`; variáveis de ambiente do `.mcp.json` vs. `required-vars.md` | Prevenção de divergência infra↔governança |
